...
from store_app.models import CustomerNotification, DeliveryAgent, Order
...
def agent_login(request):
    ...
    if request.method == "POST":
        ...
        if user and agent and user.pk == agent.user_id:
            ...
            return redirect(next_page or "agent:dashboard")
        messages.error(request, "Invalid credentials or inactive delivery-agent account.")
    return render(request, "agent/login.html", {"next": request.GET.get("next")})
